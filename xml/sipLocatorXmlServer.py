...
def main():
    ...
    if sipLocatorConfig.XML_LOG_FILE == "" or sipLocatorConfig.XML_LOG_FILE == None:
            sipLocatorConfig.LOG_FILENAME = "logs/sipLocatorXmlDefault.log"

    logging.basicConfig(filename=sipLocatorConfig.XML_LOG_FILE, level=logging.INFO, format='%(asctime)s.%(msecs).03d %(levelname)s %(message)s', datefmt='%m/%d/%Y %I:%M:%S') 
